...
//mzky
func (m *MySqlBackup) ExportTableToFile(fileName, tableName string) error {
	fileObj, err := os.Create(fileName)
	if err != nil {
		log.Error("ExportToFile:%v", err)
		return err
	...
	if m.exportInfo.ExportTableStructure || m.exportInfo.ExportRows {
		...
		m.currentTableIndex++
		m.totalRowsInCurrentTable = m.dataBase.GetTable(tableName).GetTableRows(m.db)
		log.Warn("%v表行数:%+v", tableName, m.totalRowsInCurrentTable)
		if m.totalRowsInCurrentTable == 0 {
			return errors.New("查看数据库对应的表" + tableName + "是否存在数据")
		}
...
//通过文件导入
func (m *MySqlBackup) ImportFromFile(fileName string) error {
	...
	return m.Import_Start()
}

func (m *MySqlBackup) Import_Start() error {
...
func (m *MySqlBackup) Import_ProcessLine(line string) error {
	...
	case NextImportAction_AppendLineAndExecute:
		return m.Import_AppendLineAndExecute(line)
		break
	}
	return nil
}
...
func (m *MySqlBackup) Import_AppendLineAndExecute(line string) error {
	m.buffImport.WriteString(line)
	m.buffImport.WriteString("\r\n")
